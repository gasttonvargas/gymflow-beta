...
BETA VERSION - Diseñado para demostración comercial

Stack: Flask + SQLite + HTML/CSS
Autor: Sistema diseñado para gimnasios pequeños (80-200 alumnos)
"""
...
def init_db():
    ...
    y datos de ejemplo para la demo
    """
    conn = get_db()
    cursor = conn.cursor()
    ...
    # Tabla de alumnos (ACTUALIZADA con usuario/contraseña)
    cursor.execute('''
    ...
    # Datos de ejemplo - Alumnos (con usuario y contraseña)
    alumnos_ejemplo = [
        ...
        ('Lucía Sánchez', '381-555-7890', 'lucia.sanchez@email.com', 'lucia', 'gym123', '2026-01-05', '2026-03-05', ''),
        ('Roberto Silva', '381-555-2345', 'roberto.silva@email.com', 'roberto', 'gym123', '2025-09-15', '2026-01-10', ''),
        ('Sofía Ramírez', '381-555-6789', 'sofia.ramirez@email.com', 'sofia', 'gym123', '2026-01-15', '2026-03-15', 'Objetivo: ganar masa'),
        ('Diego Torres', '381-555-1111', 'diego.torres@email.com', 'diego', 'gym123', '2025-12-20', '2026-02-20', ''),
    ]
    
    for alumno in alumnos_ejemplo:
        cursor.execute('''
            INSERT OR IGNORE INTO alumnos (nombre, telefono, email, username, password, fecha_inicio, fecha_vencimiento, notas)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', alumno)
    
    # Datos de ejemplo - Rutinas
    rutina_hipertrofia = json.dumps([
        {
            "nombre": "Sentadillas",
            "series": 4,
            "repeticiones": "12",
        ...
        },
        {
            ...
            "repeticiones": "12",
            "descanso": "90 seg",
            "notas": "Espalda recta, empujar con talones"
        },
        {
            ...
            "repeticiones": "12",
            "descanso": "90 seg",
            "notas": "Llevar codos hacia atrás"
        }
    ])
    
    rutina_principiante = json.dumps([
        {
            "nombre": "Caminar en cinta",
            "series": 1,
            "repeticiones": "15 min",
            "descanso": "-",
            "notas": "Velocidad moderada para entrada en calor"
        },
        {
        ...
        },
        {
            ...
            "repeticiones": "20",
            "descanso": "45 seg",
            "notas": "Subir solo con abdomen, no con cuello"
        }
    ])
    
    rutinas_ejemplo = [
        (1, 'Hipertrofia - Febrero 2026', '2026-01-27', '2026-03-27', rutina_hipertrofia),
        (4, 'Adaptación - Principiante', '2026-01-20', '2026-02-05', rutina_principiante),
        (7, 'Fuerza + Volumen', '2026-01-15', '2026-03-15', rutina_hipertrofia),
    ]
    
    for rutina in rutinas_ejemplo:
        cursor.execute('''
            INSERT OR IGNORE INTO rutinas (alumno_id, nombre, fecha_inicio, fecha_fin, ejercicios)
            VALUES (?, ?, ?, ?, ?)
        ''', rutina)
...
def calcular_estado_alumno(fecha_vencimiento_str):
    ...
    Retorna: 'activo', 'por_vencer' (7 días o menos), 'vencido'
    """
...
def calcular_estado_rutina(fecha_fin_str):
    ...
    Retorna: 'vigente', 'por_vencer' (7 días o menos), 'vencida'
    """
...
# ============================================
# RUTAS - LANDING PAGE Y AUTENTICACIÓN
# ============================================
...
def marketing():
    ...
    # Alumnos vencidos (para segmentación)
    alumnos_vencidos = 0
...
def marketing_nueva():
    ...
    # Calcular estadísticas para destinatarios
    conn = get_db()
...
def alumno_eliminar(id):
    """Eliminar un alumno (con confirmación en el template)"""
    if 'user_id' not in session:
        return redirect(url_for('login'))
...
def rutina_editar(id):
    ...
    # Parsear ejercicios para el formulario
    ejercicios = json.loads(rutina['ejercicios'])
...
# ============================================
# RUTAS - VISTA DEL ALUMNO (PÚBLICA)
# ============================================

@app.route('/alumno/ver/<int:id>')
def alumno_ver(id):
    """
    Vista pública para que el alumno vea su rutina
    NO requiere login - acceso por link directo
    """
    conn = get_db()
    ...
    # Obtener rutina vigente (la más reciente que no haya vencido)
    rutina = conn.execute('''
        SELECT * FROM rutinas 
        WHERE alumno_id = ? 
        ORDER BY fecha_fin DESC 
...
if __name__ == '__main__':
    ...
    init_db()
    print("=" * 50)
    print("🏋️  GymFlow Beta - Sistema Iniciado")
    print("=" * 50)
    ...
    # Ejecutar aplicación en modo debug (solo para desarrollo)
